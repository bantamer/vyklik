...
def test_is_working_inside_window():
    ...
    assert is_working(datetime(2026, 5, 4, 10, 0, tzinfo=UTC), schedule) is True


def test_is_working_outside_hours():
    schedule = {0: (8, 16)}
    # 06:00 UTC = 08:00 Warsaw — start is inclusive
    assert is_working(datetime(2026, 5, 4, 6, 0, tzinfo=UTC), schedule) is True
    # 14:00 UTC = 16:00 Warsaw — end is exclusive
    assert is_working(datetime(2026, 5, 4, 14, 0, tzinfo=UTC), schedule) is False


def test_is_working_off_day():
    schedule = {0: (8, 16)}  # only Monday
    # Tuesday
    assert is_working(datetime(2026, 5, 5, 10, 0, tzinfo=UTC), schedule) is False
